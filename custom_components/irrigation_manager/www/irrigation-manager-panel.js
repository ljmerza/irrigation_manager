...
`,oe="irrigation_manager",ae=[24,48,72],le=t=>String(t&&"object"==typeof t&&"message"in t?t.message:t),ce=(t,e)=>{const{[e]:i,...s}=t;return s},de=t=>{history.pushState(null,"",t),window.dispatchEvent(new CustomEvent("location-changed",{detail:{replace:!1}}))};let he=class extends ot{constructor(){super(...arguments),this.narrow=!1,this._pending={},this._actionErrors={},this._evaluations={},this._histories={},this._now=Date.now(),this._subscribeFailedAt=0,this._handleReconnect=()=>{if(!this._unsubscribe)return this._subscribeFailedAt=0,void this._subscribe();this._refresh()},this._closeDialog=()=>{this._dialog=void 0},this._handleKeydown=t=>{"Escape"===t.key&&this._dialog&&(this._dialog=void 0)}}connectedCallback(){super.connectedCallback(),this._subscribe(),this._timer=window.setInterval(()=>this._tick(),1e3),window.addEventListener("keydown",this._handleKeydown)}disconnectedCallback(){super.disconnectedCallback(),window.clearInterval(this._timer),this._timer=void 0,window.removeEventListener("keydown",this._handleKeydown),this._unsubscribeAll()}shouldUpdate(t){if(this._subscribe(),1!==t.size||!t.has("hass"))return!0;const e=t.get("hass"),i=this.hass;return!(e&&i&&this._schedules)||(e.locale!==i.locale||e.language!==i.language||e.config!==i.config||e.dockedSidebar!==i.dockedSidebar||this._watchedEntities().some(t=>e.states[t]!==i.states[t]))}render(){const t=this.hass,e=this.narrow||"always_hidden"===t?.dockedSidebar;return V`
      <div class="toolbar">
        ...
        <button class="toolbar-button" @click=${this._addSchedule}>
          ${ne(Zt)}<span>Add schedule</span>
        </button>
      </div>
      <main class="content">
        ${this._loadError?V`<div class="banner error" role="alert">${this._loadError}</div>`:B}
        ${this._renderGlobal()}
        ${this._renderBody(t)}
      </main>
      ${this._renderDialog()}
    `}_renderGlobal(){const t=this._schedules??[];if(!t.length)return B;const e=t.filter(t=>t.paused).length,i=t.some(t=>t.running),s=void 0!==this._globalPending;return V`
      ${e?V`<div class="banner warn" role="status">
            ${ne(Yt)}${e===t.length?"All schedules are paused. Scheduled runs do nothing until you resume.":`${e} of ${t.length} schedules are paused.`}
          </div>`:B}
      ${this._globalError?V`<div class="banner error" role="alert">${this._globalError}</div>`:B}
    ...
    `}_renderBody(t){return t&&void 0!==this._schedules?0===this._schedules.length?V`
        <div class="empty">
          <div class="empty-icon">${ne(Xt)}</div>
    ...
    `:this._loadError?B:V`<div class="empty">Loading schedules…</div>`}_renderSchedule(t,e){const i=e.config??{},s=e.entry_id,n=this._pending[s],r=void 0!==n,o=this._actionErrors[s],a=qt(t,i),l=((t,e)=>It(t,e.last_details))(t,e),c=e.unclosed_zones??[],d=this._evaluations[s];return V`
      <section class="card ${e.enabled?"":"is-disabled"}">
        <header class="card-header">
          <div class="heading">
            <h2>${e.name}</h2>
            <span class="badge ${h=e.status,wt[h]?.tone??"muted"}">${At(e.status)}</span>
            ${e.paused&&"paused"!==e.status?V`<span class="chip warn">paused</span>`:B}
          </div>
        ...
        <dl class="rows">
          ...
          <dd>
            <div>${jt(t,i)}</div>
            <div class="muted">${((t,e)=>{if("hourly"===e.frequency)return"Every day";if("time"===e.start_mode)return e.start_time?`Starts at ${Nt(t,e.start_time)}`:"Fixed time";const i="sunset"===e.start_mode?"sunset":"sunrise",s=e.sun_offset_minutes??0;return 0===s?`Finishes at ${i}`:s>0?`Finishes ${Ot(s)} before ${i}`:`Finishes ${Ot(-s)} after ${i}`})(t,i)}</div>
          </dd>
        ...
        ${d?this._renderEvaluation(t,e,d):B}

        ${o?V`<div class="banner error card-banner" role="alert">${o}</div>`:B}
        ...
        <footer class="actions">
          ...
          >
            ${e.skip_next?V`${ne(Jt)}Cancel skip`:V`${ne(Gt)}Skip next`}
          </button>
          <button class="action" ?disabled=${r} @click=${()=>this._evaluate(e)}>
            ${ne(te)}${"evaluate"===n?"Checking…":"Check now"}
          </button>
          ${i.ai_task_entity?V`
                <button
                  class="action"
                ...
                >
                  ${ne(se)}${"generate_report"===n?"Writing report…":"Weekly report"}
                </button>
                <button
                  class="action"
                  ?disabled=${r}
                ...
                >
                  ${ne(se)}${"explain_skips"===n?"Explaining…":"Explain skips"}
                </button>
              `:B}
    ...
    `;var h}_renderNextRun(t,e){return e.enabled?e.next_run?V`
      ...
      ${!1===e.next_run_scheduled?V`<span class="chip" title="Waters only if a moisture sensor reads below the threshold"
            >moisture check</span
          >`:B}
      ${e.skip_next?V`<span class="chip warn">next watering skipped</span>`:B}
      ${e.paused?V`<span class="chip warn">paused</span>`:B}
    ...
    `}_renderZones(t,e,i){const s=e.config??{},n=s.zones??[];return n.length?V`
      ...
      ${n.length>1?V`<div class="muted">${(t=>"concurrent"===t.zone_mode?"All zones at once":"One zone at a time")(s)}</div>`:B}
    `:V`<span class="muted">No zones</span>`}_renderRunning(t,e){let i=e.active_zones??[];return!i.length&&e.current_zone&&(i=[{entity_id:e.current_zone,ends_at:e.current_zone_ends_at}]),V`
      <div class="running-block">
        <div class="running-title">${ne(Xt)}Watering</div>
    ...
    `}_renderLastRun(t,e){if(!e.last_run_start)return V`<span class="muted">Never</span>`;const i=e.last_run_total_minutes,s=e.zone_results??[];return V`
      <div>
        ${Lt(t,e.last_run_start)}${e.last_run_end?` – ${Rt(t,e.last_run_end)}`:""}
      </div>
      ${null!=i?V`<div class="muted">${Pt(t,i,1)} min total</div>`:B}
      ${s.length?V`<ul class="zone-list">
            ${s.map(e=>V`<li class=${e.error?"error":""}>
                <span>
                  ${kt(t,e.entity_id)}
                  ${e.error?V`<span class="zone-error">${e.error}</span>`:B}
                </span>
    ...
    `}_renderHistory(t,e,i){const s=this._histories[e.entry_id],n=s??e.history??[];return n.length?V`
      ...
      ${s?V`<button
            class="mini"
            @click=${()=>this._histories=ce(this._histories,e.entry_id)}
          >
            Show less
    ...
    `:V`<span class="muted">No runs or skips yet</span>`}_renderEvaluation(t,e,i){return V`
      <div class="check-block">
        <div class="check-title">
          <span>Check at ${Rt(t,i.at)}</span>
          <button
            class="icon-button small"
            aria-label="Close check result"
            @click=${()=>this._evaluations=ce(this._evaluations,e.entry_id)}
          >
            ${ne(ee)}
          </button>
        </div>
        ${((t,e)=>{const{decision:i}=e,s=[];return i.water?s.push({text:"Conditions allow watering now",tone:"ok"}):i.status?s.push({text:`Would skip: ${At(i.status)}`,tone:"warn"}):s.push({text:"Would not water: not a schedule day and no sensor is dry",tone:"muted"}),i.retry&&s.push({text:"Would re-check every 2 min until the maximum delay",tone:"info"}),e.scheduled||s.push({text:"Checked as a moisture check day",tone:"muted"}),e.rain_delay_until&&s.push({text:`Rain delay until ${Lt(t,e.rain_delay_until)}: scheduled runs are skipped`,tone:"warn"}),e.paused&&s.push({text:"Paused: scheduled runs do nothing",tone:"warn"}),e.enabled||s.push({text:"Schedule is off",tone:"muted"}),[...s,...It(t,i.details)]})(t,i).map(t=>V`<div class="detail ${t.tone??""}">${t.text}</div>`)}
      </div>
    ...
    `:B}_subscribe(){const t=this.hass;if(!t||!this.isConnected)return;if(this._connection===t.connection&&(this._unsubscribe||Date.now()-this._subscribeFailedAt<3e4))return;this._unsubscribeAll();const e=t.connection;this._connection=e,e.addEventListener("ready",this._handleReconnect);const i=e.subscribeMessage(t=>{this._schedules=t.schedules,this._loadError=void 0},{type:`${oe}/subscribe`});this._unsubscribe=i,i.catch(t=>{this._unsubscribe===i&&(this._unsubscribe=void 0,this._subscribeFailedAt=Date.now(),this._loadError=`Could not load schedules: ${le(t)}`)})}_unsubscribeAll(){this._connection?.removeEventListener("ready",this._handleReconnect),this._connection=void 0;const t=this._unsubscribe;this._unsubscribe=void 0,t?.then(t=>t()).catch(()=>{})}async _refresh(){if(this.hass)try{const t=await this.hass.callWS({type:`${oe}/schedules`});this._schedules=t.schedules,this._loadError=void 0}catch(t){}}_watchedEntities(){const t=new Set,e=e=>{e&&t.add(e)};for(const t of this._schedules??[]){const i=t.config??{};i.zones?.forEach(t=>e(t.entity_id)),i.moisture_sensors?.forEach(e),i.rain_sensors?.forEach(e),i.weather_entities?.forEach(e),i.occupancy_entities?.forEach(e),e(i.rain_sensor),e(i.weather_entity),e(i.temperature_sensor),e(i.wind_sensor);const s=t.last_details??{};e(s.moisture?.lowest_entity),(s.moisture?.unavailable_entities??[]).forEach(e),(s.occupancy?.occupied??[]).forEach(e),Object.keys(s.rain?.stations??{}).forEach(e),t.unclosed_zones?.forEach(e),t.zone_results?.forEach(t=>e(t.entity_id)),t.active_zones?.forEach(t=>e(t.entity_id))}return[...t]}_tick(){const t=Date.now();(this._schedules?.some(t=>t.running)||t-this._now>=3e4)&&(this._now=t)}async _request(t,e,i={}){const s=this.hass,n=t.entry_id;if(s&&void 0===this._pending[n]){this._pending={...this._pending,[n]:e},this._actionErrors=ce(this._actionErrors,n);try{return await s.callWS({type:`${oe}/${e}`,entry_id:n,...i})}catch(t){return void(this._actionErrors={...this._actionErrors,[n]:le(t)})}finally{this._pending=ce(this._pending,n)}}}async _action(t,e,i={}){const s=await this._request(t,e,i);s&&this._schedules&&(this._schedules=this._schedules.map(e=>e.entry_id===t.entry_id?s:e))}async _evaluate(t){const e=await this._request(t,"evaluate");e&&(this._evaluations={...this._evaluations,[t.entry_id]:e})}async _loadHistory(t){const e=await this._request(t,"history",{limit:100});e&&(this._histories={...this._histories,[t.entry_id]:e.history})}async _aiText(t,e){const i=await this._request(t,e);i&&(this._dialog={title:"generate_report"===e?`${t.name}: weekly report`:`${t.name}: skipped runs`,text:i.text})}_runZone(t,e,i){const s=window.prompt(`Run ${kt(t,i.entity_id)} for how many minutes?`,String(i.minutes));if(null===s)return;const n=Number(s.trim());!Number.isInteger(n)||n<1||n>180?this._actionErrors={...this._actionErrors,[e.entry_id]:"Enter whole minutes from 1 to 180."}:this._action(e,"run_zone",{zone:i.entity_id,minutes:n})}async _globalAction(t){const e=this.hass;if(e&&void 0===this._globalPending&&("stop_all"!==t||window.confirm("Stop every active run?"))){this._globalPending=t,this._globalError=void 0;try{const i=await e.callWS({type:`${oe}/${t}`});i?.schedules&&(this._schedules=i.schedules)}catch(t){this._globalError=le(t)}finally{this._globalPending=void 0}}}_toggleEnabled(t,e){const i=e.target,s=i.checked;i.checked=t.enabled,this._action(t,"set_enabled",{enabled:s})}_toggleMenu(){this.dispatchEvent(new CustomEvent("hass-toggle-menu",{bubbles:!0,composed:!0}))}_addSchedule(){de(`/_my_redirect/config_flow_start?domain=${oe}`)}_editSchedule(t){de(`/config/integrations/integration/${oe}#config_entry=${t.entry_id}`)}};he.styles=re,t([dt({attribute:!1})],he.prototype,"hass",void 0),t([dt({type:Boolean,reflect:!0})],he.prototype,"narrow",void 0),t([dt({attribute:!1})],he.prototype,"route",void 0),t([dt({attribute:!1})],he.prototype,"panel",void 0),t([ht()],he.prototype,"_schedules",void 0),t([ht()],he.prototype,"_loadError",void 0),t([ht()],he.prototype,"_pending",void 0),t([ht()],he.prototype,"_actionErrors",void 0),t([ht()],he.prototype,"_evaluations",void 0),t([ht()],he.prototype,"_histories",void 0),t([ht()],he.prototype,"_globalPending",void 0),t([ht()],he.prototype,"_globalError",void 0),t([ht()],he.prototype,"_dialog",void 0),t([ht()],he.prototype,"_now",void 0),he=t([(t=>(e,i)=>{void 0!==i?i.addInitializer(()=>{customElements.define(t,e)}):customElements.define(t,e)})("irrigation-manager-panel")],he);export{he as IrrigationManagerPanel};
